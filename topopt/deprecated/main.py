...
targets = np.array([0.2, 0.3, 0.4, 0.5]) 

@jax.jit
def fem_loss(model, coords, target_density):

    rho = jax.nn.sigmoid(model(coords))
    # jax.debug.print("Magnitude of rho: {}", np.linalg.norm(rho))
    # has_nan = np.sum(np.isnan(rho))
    # jax.debug.print("{}",has_nan)
    ...
    vol_penalty = 200 * (np.mean(rho) - target_density)**2
...
trained_siren, opt_state = train_siren(siren, coords, num_epochs=200, lr=1e-4)
