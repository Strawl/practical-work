...
def save_normalized_image(src: Path, dst: Path, *, width: float = 8, height: float = 4):
    ...
    ax.imshow(arr, cmap="gray_r", interpolation="nearest", aspect="equal")
    ax.set_axis_off()
...
def main():
    ...
    # Normalize all images used in this figure.
    labels = [
        "mma_scale3_projected",
        "mma_scale15_projected",
        "jax_tounn_scale3",
        "jax_tounn_scale15",
        "siren_model_1_scale3",
        "siren_model_1_scale15",
    ]
    rows = [df.loc[df["label"] == label].iloc[0] for label in labels]
    for row in rows:
        save_normalized_image(EXPERIMENT_DIR / row["image_file"], NORMALIZED_DIR / row["image_file"])

    mma = df.loc[df["label"] == "mma_scale3_projected"].iloc[0]
    mma15 = df.loc[df["label"] == "mma_scale15_projected"].iloc[0]
    jt3 = df.loc[df["label"] == "jax_tounn_scale3"].iloc[0]
    jt15 = df.loc[df["label"] == "jax_tounn_scale15"].iloc[0]
    s3 = df.loc[df["label"] == "siren_model_1_scale3"].iloc[0]
    s15 = df.loc[df["label"] == "siren_model_1_scale15"].iloc[0]
